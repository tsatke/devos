...
impl<T: Copy> Read<T> for RingBuffer<T> {
    fn read(&mut self, buf: &mut [T]) -> Result<ReadResult, ReadError> {
        ...
        Ok(match NonZeroUsize::new(read) {
            None => ReadResult::TryAgain,
            Some(n) => ReadResult::Read(n),
        })
    }
}
...
impl<T: Copy> Write<T> for RingBuffer<T> {
    fn write(&mut self, buf: &[T]) -> Result<WriteResult, WriteError> {
        ...
        Ok(match NonZeroUsize::new(written) {
            None => WriteResult::TryAgain,
            Some(n) => WriteResult::Written(n),
        })
    }
}
...
mod tests {
    use crate::foundation::io::{Read, ReadResult, Write, WriteResult};
    ...
    fn test_ring_buffer() {
        ...
        assert_eq!(
            ...
            WriteResult::Written(NonZeroUsize::new(13).unwrap())
        );
        ...
        assert_eq!(
            ...
            ReadResult::Read(NonZeroUsize::new(5).unwrap())
        );
        ...
        assert_eq!(
            buf.read(&mut read_buf).unwrap(),
            ReadResult::Read(NonZeroUsize::new(3).unwrap())
        );
        assert_eq!(&read_buf, b"ld!or"); // "ld!" is left in the buffer, "or" is from the previous read

        assert_eq!(buf.read(&mut read_buf).unwrap(), ReadResult::TryAgain);
        ...
        assert_eq!(
            buf.read(&mut read_buf).unwrap(),
            ReadResult::Read(NonZeroUsize::new(13).unwrap())
        );
